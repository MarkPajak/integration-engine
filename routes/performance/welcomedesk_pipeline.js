...
Team.aggregate([
	...
	{
        $lookup: {
           from: "kpi_logs", //lower case and pleural
    ...
    {
       $unwind: "$visits"  //this bundless ALL visits into every document - need to run a match on the dates 
    },

//{ $match: { 'visits.museum_id' ,  "BMAG"}}	,
	{ 
	   $group: {
		  _id: {
			 year : { $year : "$visits.date_value" },
			 month : { $month : "$visits.date_value" },
			 venue:'$visits.museum_id',
			 ATV:"$ATV",
			 number_transactions:"$number_transactions",
			 total:"$total"
		  } ,
		  visits: { $sum: '$visits.value' } ,
