var save_data_to_database = function (keys,options){
	...
	var Shopify_vendor= require('../../../models/shopify/Shopify_vendor_report.js');
	...
	self.remove_data = function(cb){
			...
			Shopify_vendor.find({shop_id:keys.shopify_store
	...
	var i=0	
		...
		self.remove_data( function(){
				...
				async.forEach(google_data, function(_product, cbb) {				
								 ...
								 new_product_month.month_vendor_id=options.year+options.month+_product.vendor
								 ...
								 new_product_month.discounts=_product.discounts ||0	        //>>
							...
							//	 new_product_month.order_cost=_product.order_cost
								...
								var shopify_vendor = new Shopify_vendor(new_product_month);
								shopify_vendor.save(function(err){	
